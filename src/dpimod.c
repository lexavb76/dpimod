...
#define BASE_IPHDR_LEN         \
	sizeof(struct iphdr) / \
		4 // Base IP header length without options in 32-bit words

char msg_start[] =
	"[dpimod started] Drop IP packets with options set (RFC 1108):\n  130/0x82	SEC	Security (RIPSO)\n"
	"  133/0x85	E-SEC	Extended Security (RIPSO)";

char msg_finish[] = "[dpimod finished]";

unsigned int dpi_hook_handler(void *priv, struct sk_buff *skb,
			      const struct nf_hook_state *state);
void check_ip_options(struct iphdr *iph, const struct nf_hook_state *state,
		      int *action, int action_todo);
...
int init_module(void)
{
	...
	nf_register_net_hook(&init_net,
			     &dpi_opts); // init_net: default network namespace
	dpi_opts.hooknum = NF_INET_LOCAL_OUT;
	nf_register_net_hook(&init_net,
			     &dpi_opts); // The same for output packets
	return 0;
}
...
void cleanup_module(void)
{
	...
	dpi_opts.hooknum =
		NF_INET_PRE_ROUTING; // Unregister in revers order (just for convinience)
	nf_unregister_net_hook(&init_net, &dpi_opts);
		      ...
		      int *action, int action_todo)
{
	unsigned char *opt;
	int optslen;
	opt = (unsigned char *)&iph
		[1]; // Points to the first byte after Base IP Header (struct iphdr)
	optslen = 4 * (iph->ihl - BASE_IPHDR_LEN);
	if (optslen > 0) { // ip_options present
		pr_info("[dpimod] IP packet, hook %d, ip_hdr_len = %d, optlen = %x, default action: %d\n",
			state->hook, iph->ihl, optslen, *action);
		while (optslen > 0 && *opt != IPOPT_END) {
			...
			if (*opt == IPOPT_SEC ||
			    *opt == IPOPT_SEC +
					    3) { // Base and extended Security options (RFC 1108)
				*action = action_todo;
			...
			optslen -=
				(opt[1] > 0) ?
					opt[1] :
					optslen; // Stop if opt length is 0 (malformed option)
			opt += opt[1];
		}
	}
}
